...
| Captioning model | **Qwen2.5-VL (3B/7B)** | BLIP-2, LLaVA, Qwen3-VL, Molmo2 | Qwen2.5-VL is open-source, runs on a single consumer/cloud GPU, and has demonstrated use in timestamp-grounded long-video indexing — directly matching this use case. Qwen3-VL/Molmo2 are stronger but heavier and newer with less tooling maturity; noted as future-work, not adopted now. |
...
| Captioning fallback | Rule-based template captions as an acceptable v1 | Requiring VLM captioning from day one | Keeps a working, demoable pipeline possible even if VLM integration slips, without blocking the retrieval/QA half of the system. |
| Vector database | **ChromaDB** | Pinecone, Weaviate, Qdrant | Free, local, fast to iterate with, and already used in a prior project (RAGHive) — no new tooling to learn mid-build. Production-scale alternatives are documented as a known future consideration, not implemented now. |
